...
class WC_Vivawallet_Apm extends WC_Vivawallet_Payment_Gateway {
	...
	public function admin_notices() {
		...
		if ( 'no' === $this->main_plugin_enabled ) {
			$error = __( 'Viva Wallet: The plugin of Viva Wallet for WooCommerce must be enabled. Please enable it and add your credentials.', 'viva-wallet-for-woocommerce' );
		...
		if ( empty( $this->credentials['token_front'] ) ) {
			$error = __( 'Viva Wallet: Your credentials are NOT valid. Please check your credentials in the main settings page of Viva Wallet for WooCommerce!', 'viva-wallet-for-woocommerce' );
			echo '<div class="error"><p><b>' . esc_html( $error ) . '</b></p></div>';
			return;
		}
