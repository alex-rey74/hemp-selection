...
if ( ! class_exists( 'WC_Vivawallet_Helper' ) ) {
	...
	class WC_Vivawallet_Helper {
		...
		public static function update_viva_wallet_webhook_url() {
			...
			$credentials = WC_Vivawallet_Credentials::get_credentials();

			$access_token = ( 'yes' === $test_mode ) ? $credentials['demo_token_back'] : $credentials['live_token_back'];
		...
		public static function get_alternative_payments_methods_key() {
			...
			if ( isset( $_SESSION['VW_APM_KEY'] ) && ! empty( $_SESSION['VW_APM_KEY'] ) ) {
				return $_SESSION['VW_APM_KEY'];
			}
		...
		public static function get_payment_methods( $test_mode ) {
			...
			if ( isset( $_SESSION['VW_AVAILABLE_APM'] ) && ! empty( $_SESSION['VW_AVAILABLE_APM'] ) ) {
				return $_SESSION['VW_AVAILABLE_APM'];
			}

			$credentials = WC_Vivawallet_Credentials::get_credentials();

			$token = ( 'yes' === $test_mode ) ? $credentials['demo_token_back'] : $credentials['live_token_back'];
			...
			if ( ! is_wp_error( $result ) && isset( $result['response']['code'] ) && 200 === $result['response']['code'] ) {
				...
				$_SESSION['VW_AVAILABLE_APM'] = $result_body;
				return $result_body;
			}
		 ...
		 *
		 * @param string $currency_symbol VW currency type (ISO).
		 *
		...
		public static function transaction_api_call( $post_args, $access_token, $test_mode, $recurring_trans_id = '' ) {
			...
			$user_agent .= ' WP/' . $wp_version . ' WC/' . $woocommerce->version . ' VW/' . WC_VIVAWALLET_VERSION;
