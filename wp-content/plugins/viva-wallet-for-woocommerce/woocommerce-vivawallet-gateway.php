 ...
 */
define( 'WC_VIVAWALLET_VERSION', '1.4.1' );
	...
	if ( ! class_exists( 'PGWC_Vivawallet' ) ) :
		...
		class PGWC_Vivawallet {
			...
			private function update_version_in_db( $viva_settings, $existing ) {
				...
				// get a new token.
				WC_Vivawallet_Credentials::get_credentials( true );
			...
			public function plugin_row_meta( $links, $file ) {
				if ( plugin_basename( __FILE__ ) === $file ) {
					$row_meta = array(
						'docs'    => '<a href="' . esc_url( 'https://docs.woocommerce.com/document/viva-wallet-for-woocommerce' ) . '" aria-label="' . esc_attr__( 'View Viva Wallet for WooCommerce plugin documentation', 'viva-wallet-for-woocommerce' ) . '">' . esc_html__( 'Documentation', 'viva-wallet-for-woocommerce' ) . '</a>',
